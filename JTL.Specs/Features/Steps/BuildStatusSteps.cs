...
namespace JTL.Specs.Features.Steps
{
    ...
    public class BuildStatusSteps
    {
        ...
        {
            var mockRssFeed = GetMockRssFeed(GreenCurrentRedPrevious);

            _buildMonitor = new BuildMonitor(mockRssFeed);
        }

        [When(@"the current build failed")]
        public void WhenTheCurrentBuildFailed()
        {
            var mockRssFeed = GetMockRssFeed(RedCurrentGreenPrevious);
        ...
        private static IJenkinsRssFeed GetMockRssFeed(string xmlResponse)
        {
            var mockFeedReader = Substitute.For<IJenkinsRssFeed>();
            mockFeedReader.Read().Returns(xmlResponse);
